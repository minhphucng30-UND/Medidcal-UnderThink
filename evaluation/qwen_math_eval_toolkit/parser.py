...
def extract_answer(pred_str, data_name, use_last_number=True):
    if data_name == 'multiple_choice':
        pred_str = pred_str.replace("\u043a\u0438", "")
        if len(choice_answer_clean(pred_str)) == 1:
            pred = choice_answer_clean(pred_str[-64:])
            return pred
        else:
            return ""
    # Bold wraps "Final Answer: Yes/No" (entire phrase in **...**)
    else:
        if "\n\n**Final Answer: Yes**" in pred_str or "\n**Final Answer: Yes**" in pred_str:
            pred = "Yes"
            return pred
        elif "\n\n**Answer: No**" in pred_str or "\n\n**Answer:** No" in pred_str:
            pred = "No"
            return pred
        elif "\n\n**Answer: Yes**" in pred_str or "\n\n**Answer:** Yes" in pred_str:
            pred = "Yes"
            return pred
        elif "\n\n**Final Answer: No**" in pred_str or "\n**Final Answer: No**" in pred_str:
            pred = "No"
            return pred
        # Bold ends after the colon; Yes/No follows outside **...**
        elif "\n\n**Final Answer:** Yes" in pred_str or "\n**Final Answer:** Yes" in pred_str or "\n\n**Yes**" in pred_str:
            pred = "Yes"
            return pred
        elif "\n\n**Final Answer:** No" in pred_str or "\n**Final Answer:** No" in pred_str or "\n\n**No**" in pred_str:
            pred = "No"
            return pred
        elif pred_str.endswith('\n\nYes.') or pred_str.endswith('Yes'):
            pred = "Yes"
            return pred
        elif pred_str.endswith('\n\nNo.'):
            pred = "No"
        ...
        elif "\\boxed{\\text{yes}}" in pred_str:
            pred = "Yes"
            return pred
        elif "\\boxed{\\text{no}}" in pred_str:
            pred = "No"
            return pred
            
        # if match:
        #     pred = match.group(1).strip()
        #     if "yes" not in pred.lower() and "no" not in pred.lower():
        #         print("check", pred_str[-64:].lower())
        ...
        elif "final answer is" in pred_str.lower():
